...
function Router(app, control) {
  var inProc = _.identity;
  var outProc = _.identity;
  ...
  var r = function(evt) {
    ...
    if (!a) { return Promise.rejected(new Error('router: not mounted')); }
    ...
    var h = a[method];
    if (!h) {
      return error(msg.id, ERR_METHOD_NOT_FOUND, 'method not found', src);
    }
    ...
    return Promise.toPromise(ret)
      .defer(toJSON)
      .done(function(res) { respond(msg.id, res, src); })
      .fail(function(err) { error(msg.id, ERR_INTERNAL, err, src); });
  };
  ...
  function error(id, code, err, src) {
    // LOG.error('response', id, err);
    var message = Promise.toErrorMessage(err);
    outProc({ id: id, error: {
      code: code,
      message: message
    } }, src);
    return Promise.rejected(new Error(message));
  }
  ...
  r.unmount = function() {
    // if (!s || !a) { return; }
    if (!a) { return; }
    inProc = outProc = function() {
      // LOG.error('router: unmouted');
    };
    // s.stopListening();
    a = null;
    // s = null;
...
R7.loadIframe = function() {};
R7.clearIframe = function() {};
},{"lodash":1}]},{},[2])