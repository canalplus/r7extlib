	...
	var R7IFrame = function () {
	  function R7IFrame(options) {
	    ...
	    if (!this.iframe) {
	      ...
	      var style = _extends({}, IFRAME_STYLE, options.style);
...
/***/ function(module, exports) {
	...
	if (hasSessionStorage()) {
	  ...
	  __history = __history || _extends({}, __defaultHistory);
...
/***/ function(module, exports, __webpack_require__) {
	...
	function _addStreamListener(type, callback, context) {
	...
	function _removeStreamListener(type) {
	  if (type === 'focus' || type === 'blur') {
	...
	function _loadIframe(options, callback, context) {
	  if (_iframe) {
	    console.error('iframe: already loaded');
	    return;
	  }

	  var keys = _extends({}, _keys);
	  var streams = _extends({}, _streams);
	  ...
	  function restoreContext() {
	    ...
	    for (var stream in streams) {
	      _addStreamListener(stream, streams[stream]);
	    }
	  }
	...
	var R7 = function () {
	  ...
	  _createClass(R7, [{
	    ...
	    key: 'grabKey',
	    value: function grabKey(key, callback, context) {
	      var k = getKey(key);
	      if ((typeof key === 'undefined' ? 'undefined' : _typeof(key)) === 'object') {
	        send('addKeys', key);
	      } else {
	    ...
	    key: 'navigate',
	    value: function navigate(route, options, callback, context) {
	      return rpc('navigate', {
	        control: route,
	        context: options
	      }, callback, context);
	    ...
	    value: function addStreamListener(type, callback, context) {
	      _addStreamListener(type, callback, context);
	    }
	  }, {
	    key: 'removeStreamListener',
	    value: function removeStreamListener(type) {
	      _removeStreamListener(type);
	    }
	  }, {
	    key: 'loadIframe',
	    value: function loadIframe(options, callback, context) {
	      _loadIframe(options, callback, context);
	    }
